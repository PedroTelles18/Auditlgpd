...
export default function ReportsPage() {
  ...
  const sc = (s: number) => s >= 80 ? "#15803d" : s >= 60 ? "#b45309" : "#dc2626";
  const sb = (s: number) => s >= 80 ? "#f0fdf4" : s >= 60 ? "#fffbeb" : "#fef2f2";
  ...
  return (
    <div className="flex-1 flex flex-col min-w-0">
      ...
      <main className="flex-1 overflow-y-auto p-6 page-enter" style={{ background: "#f8fafc" }}>
        <div className="mb-5">
          <h1 className="text-[20px] font-extrabold mb-0.5" style={{ color: "#0f172a", letterSpacing: "-0.02em" }}>{t.reports_title}</h1>
          <p className="text-[12px]" style={{ color: "#94a3b8" }}>{t.reports_sub}</p>
        </div>
        <div className="grid grid-cols-3 gap-3 mb-5 stagger">
          {[
            { label: t.total_audits,   val: reports.length,                             color: "#0f172a" },
            { label: t.above_70,       val: reports.filter(r => r.score >= 70).length,  color: "#15803d" },
            { label: t.need_attention, val: reports.filter(r => r.score <  70).length,  color: "#dc2626" },
          ].map(({ label, val, color }) => (
            ...
            : reports.map(({ id, type, title, date, findings, score }) => (
              <div key={id} className="flex items-center gap-4 p-4 bg-white rounded-xl card-hover"
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-[13px] font-bold truncate" style={{ color: "#0f172a" }}>{title}</p>
                ...
                <button className="px-3 py-1.5 rounded-lg text-[11px] font-bold transition-colors flex-shrink-0"
                  style={{ border: "1px solid #e2e8f4", background: "#fff", color: "#475569" }}>📄 PDF</button>
              </div>
            ))}
        </div>
      </main>
