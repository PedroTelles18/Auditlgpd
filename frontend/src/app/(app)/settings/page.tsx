...
export default function SettingsPage() {
  ...
  return (
    <div className="flex-1 flex flex-col min-w-0">
      ...
      <main className="flex-1 overflow-y-auto p-6 page-enter pb-10" style={{ background: "var(--bg2)" }}>
        <div className="mb-5 max-w-2xl">
          <h1 className="text-[20px] font-extrabold mb-0.5" style={{ color: "var(--text)", letterSpacing: "-0.02em" }}>{t.settings_title}</h1>
          <p className="text-[12px]" style={{ color: "var(--text-3)" }}>{t.settings_sub}</p>
        </div>
        ...
        <div className="max-w-2xl">
          ...
          <Card className="overflow-hidden mb-3">
            <div className="flex items-center gap-2.5 px-5 py-3.5" style={{ borderBottom: "1px solid #e2e8f4", background: "var(--section-bg)" }}>
              ...
              <h2 className="text-[12px] font-extrabold uppercase tracking-wider" style={{ color: "var(--text)" }}>{t.language}</h2>
            </div>
            <div className="p-5">
                ...
                {LANGS.map(({ code, label, flag }) => (
                  <button key={code} onClick={() => setLang(code)}
                    ...
                    style={{
                      ...
                      background: lang === code ? "var(--accent-l)" : "var(--card-bg)",
              ...
              <p className="text-[11px] mt-3" style={{ color: "var(--text-3)" }}>
                Idioma atual: <strong style={{ color: "var(--text)" }}>{LANGS.find(l => l.code === lang)?.label}</strong>
              </p>
            </div>
          </Card>
