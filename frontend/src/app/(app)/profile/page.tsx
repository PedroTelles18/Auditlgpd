  ...
  const Field = ({ label, value, onChange, type="text", placeholder="" }: {
    ...
    <div className="mb-3.5">
      ...
      <input type={type} value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder}
        ...
        onFocus={e => { (e.target as HTMLInputElement).style.borderColor = "var(--accent)"; (e.target as HTMLInputElement).style.background = "var(--card-bg)"; (e.target as HTMLInputElement).style.boxShadow = "0 0 0 3px rgba(37,99,235,0.1)"; }}
        onBlur={e  => { (e.target as HTMLInputElement).style.borderColor = "var(--border)"; (e.target as HTMLInputElement).style.background = "var(--bg2)"; (e.target as HTMLInputElement).style.boxShadow = "none"; }} />
    </div>
  );
  ...
  return (
    <div className="flex-1 flex flex-col min-w-0">
      ...
      <main className="flex-1 overflow-y-auto p-6 page-enter" style={{ background: "var(--bg2)" }}>
        <div className="mb-5">
          <h1 className="text-[20px] font-extrabold mb-0.5" style={{ color: "var(--text)", letterSpacing: "-0.02em" }}>{t.profile_title}</h1>
          <p className="text-[12px]" style={{ color: "var(--text-3)" }}>Gerencie suas informações e segurança de acesso</p>
        </div>
        ...
        <Card className="p-5 mb-4 flex items-center gap-4">
          ...
          <div>
            <p className="text-[16px] font-extrabold" style={{ color: "var(--text)" }}>{user.name}</p>
            <p className="text-[12px] mb-1.5" style={{ color: "var(--text-3)" }}>{user.email}</p>
        ...
        <div className="grid lg:grid-cols-2 gap-4">
          ...
          <Card className="overflow-hidden">
            <div className="flex items-center gap-2.5 px-5 py-3.5" style={{ borderBottom: "1px solid #e2e8f4", background: "var(--section-bg)" }}>
              <div className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ background: "var(--accent-l)" }}>
                <Save size={13} style={{ color: "var(--accent)" }} />
              </div>
              <h2 className="text-[12px] font-extrabold uppercase tracking-wider" style={{ color: "var(--text)" }}>{t.personal_info}</h2>
            </div>
            <div className="p-5">
              ...
              {msg && (
                <div className="mb-3 px-3.5 py-2.5 rounded-lg text-[12px]"
                  style={{ background: msg.type==="ok" ? "var(--success-l)" : "var(--danger-l)",
                    border: `1px solid ${msg.type==="ok" ? "var(--success-m)" : "var(--danger-m)"}`,
          ...
          <Card className="overflow-hidden">
            <div className="flex items-center gap-2.5 px-5 py-3.5" style={{ borderBottom: "1px solid #e2e8f4", background: "var(--section-bg)" }}>
              <div className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ background: "var(--danger-l)" }}>
                <Shield size={13} style={{ color: "var(--danger)" }} />
              </div>
              <h2 className="text-[12px] font-extrabold uppercase tracking-wider" style={{ color: "var(--text)" }}>{t.change_password}</h2>
            </div>
            <div className="p-5">
              {[
                ...
                <div key={label} className="mb-3.5">
                  ...
                  <div className="relative">
                    <input type={show ? "text" : "password"} value={val} onChange={e => set(e.target.value)}
                      ...
                      style={{ border: "1.5px solid #e2e8f4", background: "var(--bg2)", color: "var(--text)", fontFamily: "'Plus Jakarta Sans', sans-serif" }}
                      onFocus={e => { (e.target as HTMLInputElement).style.borderColor = "var(--accent)"; (e.target as HTMLInputElement).style.background = "var(--card-bg)"; }}
                      onBlur={e  => { (e.target as HTMLInputElement).style.borderColor = "var(--border)"; (e.target as HTMLInputElement).style.background = "var(--bg2)"; }} />
              ...
              {pwMsg && (
                <div className="mb-3 px-3.5 py-2.5 rounded-lg text-[12px]"
                  style={{ background: pwMsg.type==="ok" ? "var(--success-l)" : "var(--danger-l)",
                    border: `1px solid ${pwMsg.type==="ok" ? "var(--success-m)" : "var(--danger-m)"}`,
                    color: pwMsg.type==="ok" ? "#15803d" : "var(--danger)" }}>
                  {pwMsg.text}
                </div>
              )}
