...
const modules = [
  ...
  },
  {
    ...
    description: "Verificação de configurações PostgreSQL/MySQL.",
    status: "em_breve",
    href: null,
  },
  {
    icon: Bot,
    title: "Motor de IA (Ollama)",
    description: "Análise contextualizada com DeepSeek-R1 local.",
    status: "em_breve",
    href: null,
  },
];
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    if (!loading && user) setTimeout(() => setVisible(true), 50);
  }, [user, loading, router]);
    ...
    <div className="min-h-screen bg-bg text-[#cdd9e5] font-sans">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <span className="hidden sm:block text-xs font-mono text-text-dim">
            Dashboard
          </span>
        </div>
        ...
        <div className="flex items-center gap-4">
          <div className="hidden sm:flex flex-col items-end">
            ...
            <span className="text-[10px] font-mono text-accent-soft uppercase tracking-wider">
              {user.role}
            </span>
          </div>
          <button
      ...
      {/* Content */}
      <main
        className="relative max-w-5xl mx-auto px-6 py-10 transition-all duration-500"
        style={{ opacity: visible ? 1 : 0, transform: visible ? "translateY(0)" : "translateY(12px)" }}
      >
        {/* Header */}
        <div className="mb-10">
          <p className="text-xs font-mono text-text-dim uppercase tracking-widest mb-1">
            // bem-vindo de volta
          </p>
        ...
        <div className="mb-4 flex items-center justify-between">
          ...
          <span className="text-xs font-mono text-text-dim">Sprint 2 →</span>
        </div>
        ...
        <div className="grid sm:grid-cols-3 gap-4">
          {modules.map(({ icon: Icon, title, description, status, href }) => (
            ...
            >
              {/* Top glow on hover */}
              <div
                className="absolute top-0 left-1/4 right-1/4 h-px opacity-0 group-hover:opacity-60 transition-opacity"
                style={{
              ...
              <div className="flex items-center justify-between">
                {status === "ativo" ? (
                  <span
                    className="inline-flex items-center gap-1.5 text-[10px] font-mono uppercase tracking-wider px-2 py-0.5 rounded"
        ...
        >
          ...
          <div className="space-y-3">
            {[
              ...
              { sprint: "Sprint 2", desc: "Code Analyzer + Ollama setup", done: false },
              { sprint: "Sprint 3", desc: "DB Auditor + Integração IA", done: false },
              { sprint: "Sprint 4", desc: "Relatórios PDF + Testes finais", done: false },
              ...
              <div key={sprint} className="flex items-center gap-3">
                ...
                <span className={`text-xs font-mono ${done ? "text-white" : "text-text-dim"}`}>
                  <span className="text-accent mr-2">{sprint}</span>
                  {desc}
                </span>
                {done && (
